...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "capabilities" {
		fmt.Println("restore-image firmware-ranges verify-stage1 aurora-tar")
		return
	}
	if len(os.Args) == 3 && os.Args[1] == "aurora-tar" {
		if err := decodeAuroraFile(os.Args[2]); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		return
	}
	if len(os.Args) == 5 && os.Args[1] == "firmware-ranges" {
		signalContext, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
		defer stop()
		ctx, cancel := context.WithTimeout(signalContext, 4*time.Minute)
		defer cancel()
		if err := extractFirmwareRanges(ctx, os.Args[2], os.Args[3], os.Args[4]); err != nil {
			fmt.Fprintln(os.Stderr, "omarchy-restore-image:", err)
			var fallback *rangeFallbackError
